...
export default function PrintControlsClient({
  ...
  const [summary, setSummary]           = useState<boolean>(isSummary);
  const [notesExcluded, setNotesExcluded] = useState<boolean>(excludeNotes);
  ...
  function generate() {
    ...
    if (notesExcluded) params.set('exclude_notes', '1');
  ...
  return (
    <div className="no-print mb-6">
      ...
      <div className="rounded-lg border border-gray-200 bg-gray-50 px-5 py-4">
        ...
        {/* Options — Summary + Exclude Notes */}
        <div className="mt-4 mb-2 text-[12px] font-semibold uppercase tracking-wider text-gray-500">
          Options
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {/* Summary toggle */}
          ...
          >
            <input
              ...
              className="sr-only"
              checked={summary}
              onChange={() => setSummary((v) => !v)}
            />
            {summary ? '✓ ' : ''}Summary Only
          </label>

          {/* Exclude Additional Notes toggle */}
          <label
            className={`flex cursor-pointer items-center gap-1.5 rounded-full border px-3 py-1 text-[12.5px] font-medium transition-colors select-none ${
              notesExcluded
                ? 'border-gray-800 bg-gray-800 text-white'
          ...
          >
            <input
              type="checkbox"
              className="sr-only"
              checked={notesExcluded}
              onChange={() => setNotesExcluded((v) => !v)}
            />
            {notesExcluded ? '✓ ' : ''}Exclude Additional Notes
          </label>
        </div>
